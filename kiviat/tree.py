...
from .kiviat import Arrow3D, Kiviat3D, kernel_smoothing
...
class Tree(Kiviat3D):
    ...
    def __init__(self, sample, data, bounds=None, plabels=None, range_cbar=None):
        ...
        # Distance based on HDR using KDE
        distribution = kernel_smoothing(data)
        pdf = np.exp(distribution.score_samples(data))
        idx_mode = int(np.argmax(pdf))

        centroide = data[idx_mode].reshape(1, -1)
        dists = cdist(centroide, data)[0]
        scaler = MinMaxScaler(feature_range=[0, np.pi / 4])
        self.dists = scaler.fit_transform(dists.reshape(-1, 1))

        pvalue = np.percentile(pdf, 10, interpolation='linear')
        idx_hdr_90 = int(np.argmin(abs(pdf - pvalue)))
        dists = cdist(centroide, [data[idx_hdr_90]])[0]
